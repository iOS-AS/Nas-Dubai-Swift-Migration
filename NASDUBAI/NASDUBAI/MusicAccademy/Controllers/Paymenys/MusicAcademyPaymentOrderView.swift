//
//  MusicAcademyPaymentOrderView.swift
//  NAS
//
...
@objc protocol instrumentPaymentOrderDelegate: NSObjectProtocol  {
   @objc func paymentStatus(status:String)
}
class MusicAcademyPaymentOrderView: UIViewController {
...
//   MARK: Payment
    ...
    @objc var paymentDelegate: instrumentPaymentOrderDelegate?
...
extension MusicAcademyPaymentOrderView: CardPaymentDelegate{
    func callingSDKFunction() {
        apiCalls.generateNPGAccessToken_Fee_Payment {
//            DispatchQueue.main.async {
//                self.apiCalls.createNPGOrder_Fee_Payment(amount: Int(self.total), emailAddress: DefaultWrapper.getLoggedInEmailId(), merchantOrderReference: self.timeStampStr, firstName:self.StudentName , lastName: self.StudentName, address1: "NAS Dubai", city: "Dubai", countryCode: "UAE", completion: { orderResponse in
//                DispatchQueue.main.async {
